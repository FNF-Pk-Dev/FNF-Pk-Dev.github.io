<!-- source: 函数/Psych/Tweens&TimersFunctions/index.html -->
...
> **注意**
>
> 此函数在补间动画完成时不会自动调用 `onTweenCompleted()`。您必须手动添加 onComplete 回调函数，示例如下所示；这也允许您在补间动画完成时设置自定义函数名。
...
- `loops` - 计时器将执行多少次循环；默认值：`1`。*（设置为 0 表示无限循环）*
