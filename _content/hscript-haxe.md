<!-- source: 函数/Psych/HScript&Haxe/index.html -->
...
<details>
<summary><b>导入 Haxe（参考）：</b></summary>
...
如果 **PlayState 的变量映射** 中尚不存在该变量，则创建该变量并将其设置到其中，这基本上相当于 **创建一个新的全局变量**；否则将使用新值设置该变量。 只有在 **包含该函数的 Haxe 脚本当前正在执行** 时，此操作才会生效；`getVar()` 函数也是如此。
...
> 注意：*由于某些 parentLua 语句问题，第三个参数已损坏_。*
